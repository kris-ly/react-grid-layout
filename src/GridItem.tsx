...
export default class GridItem extends React.Component<Props, State> {
    static propTypes = {
        ...
        // All optional
        minW(props: Props, propName: string) {
            const value = props[propName];
            if (typeof value !== 'number') return new Error('minWidth not Number');
            if (value > props.w || value > props.maxW) { return new Error('minWidth larger than item width/maxWidth'); }
        },

        maxW(props: Props, propName: string) {
            const value = props[propName];
            if (typeof value !== 'number') return new Error('maxWidth not Number');
            if (value < props.w || value < props.minW) { return new Error('maxWidth smaller than item width/minWidth'); }
        },

        minH(props: Props, propName: string) {
            const value = props[propName];
            if (typeof value !== 'number') return new Error('minHeight not Number');
            if (value > props.h || value > props.maxH) { return new Error('minHeight larger than item height/maxHeight'); }
        },

        maxH(props: Props, propName: string) {
            const value = props[propName];
    ...
    calcColWidth(): number {
        const {
            margin, containerPadding, containerWidth, cols,
        } = this.props;
        return (
   ...
   */
    calcWH(
        ...
        const {
            margin, maxRows, cols, rowHeight, x, y,
        } = this.props;
        const colWidth = this.calcColWidth();
   ...
   */
    mixinDraggable(child: React.ReactElement<any>): React.ReactElement<any> {
        return (
            // @ts-ignore
            <DraggableCore
   ...
   */
    onDragStart = (e: Event, {
        node,
    }: ReactDraggableCallbackData): any => {
